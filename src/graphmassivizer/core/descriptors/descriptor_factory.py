...
def create_machine_descriptor(config) -> MachineDescriptor:
    ...
    memory_max = config.get_int(
        "machine.memory.max", default=8 * 1024 * 1024 * 1024)  # 8 GB
    disk_size = config.get_int(
        "machine.disk.size", default=256 * 1024 * 1024 * 1024)  # 256 GB

    # Create HDD Descriptor
    hdd_descriptor = HDDDescriptor(size_of_hdd=disk_size)

    # Create Hardware Descriptor
    hardware_descriptor = HardwareDescriptor(
        cpu_cores=cpu_cores, size_of_ram=memory_max, hdd=hdd_descriptor)
