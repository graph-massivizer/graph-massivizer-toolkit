...
class Descriptor(ABC):
	...
	def __del__(self):
		if hasattr(self, 'zk_state_manager'): self.zk_state_manager.unregister_descriptor(self)
...
@dataclass
class SimulationMachineDescriptor:
	address: str
	host_name: str
	hardware: str
	cpu_cores: int
...
class Machine:
	...
	def parse_from_env(zookeper_state_manager: ZookeeperStateManager, prefix: str) -> "Machine":
		return Machine(
			...
			descriptor=MachineDescriptor.parse_from_env(zookeper_state_manager,prefix),
		)
