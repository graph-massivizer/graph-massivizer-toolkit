...
class Dashboard:
    ...
    def __init__(self, zookeeper_host, machine, docker_network_name) -> None:
    ...
    def explore_znodes(self, path, znodes_data_dic, znodes_hierarchy_graph_nodes, znodes_hierarchy_graph_edges, id_hierarchy_graph = '0', level=0):
        ...
        if data:
            self.logger.info('znode has data.')
            node_data = ast.literal_eval(data.decode('utf-8'))
        else:
            self.logger.info('znode has no data.')
            node_data = 'No data' 

        self.logger.info(f"Type of node_data: {type(node_data)} and its value: {node_data}")

        znodes_data_dic['Path'].append(path)
        if not node_data== 'No data':
            self.logger.info('appending znode data.')
            self.logger.info(f"node_data ID is {node_data.get('ID')}")
        ...
        children_info = f"{indent}- {path} (Data: {node_data}, Children: {stat.numChildren})"
        #znodes_hierarchy_info += children_info + '\n'
        print(children_info)
        self.logger.info(children_info)
...
def main() -> None:
    try:
        ...
        dashboard = Dashboard(zookeeper_host, machine, docker_network_name)
        config.dashboard_obj = dashboard
        dashboard.logger.info("I am Dashboard " + str(machine.ID))
        
        
        dashboard.logger.info("Starting Dashboard ... YYYYYY")
        ...
        print(f'df inside main: columns are: {containers_info_df.columns} and its len is {containers_info_df.shape[0]} dataframe is : {containers_info_df.to_string()}')
        
        dashboard.logger.info("%s", containers_info_df)
        dashboard.logger.info(f'columns of containers tables are {containers_info_df.columns}')
        ...
        dashboard.logger.info('znodes hierarchy nodes ')
        dashboard.logger.info('%s', znodes_hierarchy_graph_nodes)
        dashboard.logger.info('znodes hierarchy edges ')
        dashboard.logger.info('%s', znodes_hierarchy_graph_edges)
        ...
        workflow_DAG_path = './tests/resources/DAG.txt'
