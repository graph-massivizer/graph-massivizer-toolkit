...
def workflow_DAG_to_graph_elements():
	...
	nodes = [{"data": {"id": str(node_id), "label": str(data_dict['nodes'][node_id]['bgo']), "url": f"{metaphactoryAddress}/resource/bgoi:" + str(node_id), "implementations": data_dict['nodes'][node_id]['implementations']}, "classes": "microservice"} for node_id in data_dict['nodes'].keys()]
